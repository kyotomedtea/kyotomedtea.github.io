...
　}); //$(document).readyの終了
  ...
  function JSON2HTML(json){
    ...
    for (i = 0; i < json.length; i++) {
      ...
      HTMLtable += "<td><a href = \"" + json[i]["image1"] + "\" class = \"itemimg\" rel = \"" + json[i]["name"] + "\" title = \"" + json[i]["name"] + "\">" + json[i]["name"] + "</a></td>";
      HTMLtable += "<td><a href = \"" + json[i]["image1"] + "\" class = \"itemimg\" rel = \"" + json[i]["name"] + "\" title = \"" + json[i]["name"] + "\"><img src = \"" + json[i]["image1"] + "\"></a>"; //</td閉じてない>
      if(json[i]["image2"]){
